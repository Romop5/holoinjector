...
if [ $MACHINE_TYPE = "x86_64" ]; then
    ...
    #lldb -O "env LD_PRELOAD=`realpath ~/enhancerBin/librepeater.so`" "$DIRNAME/bin-64/supertuxkart" "$@"
    gdb -ex "set environment LD_PRELOAD=`realpath ~/enhancerBin/librepeater.so`" "$DIRNAME/bin-64/supertuxkart" "$@"
else
